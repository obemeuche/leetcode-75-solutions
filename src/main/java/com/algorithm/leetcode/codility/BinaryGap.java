package com.algorithm.leetcode.codility;
    ...
    * */
    ...
    class Solution {
        public static int solution(int N) {
            ...
            //convert to binary
            String numberToBinary = Integer.toString(N, 2);
            System.out.println(numberToBinary);
            ...
            for(int i = 0; i < numberToBinary.length(); i++){
                String c = numberToBinary.substring(i, i+1);
                if(c.startsWith("1") ){
                    if(start){
                        if(counter > maxCount){
                            maxCount = counter;
